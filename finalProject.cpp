...
public:
	...
	void multiUpdate(float elapsed){
...
public:
	...
	void multiUpdate(float elapsed, float right, float left){
		if (keys[SDL_SCANCODE_A]){
			if ((position.x - 0.5f) > left){
				position.x -= elapsed * units_a_second * 2;
			}
		}
...
int main(int argc, char *argv[])
{
	...
	Mix_Music * enemyshootsound;
	enemyshootsound = Mix_LoadMUS("enemyshoot.mp3");
	shootsound = Mix_LoadWAV("beep.wav");
	music = Mix_LoadMUS("106-super-mario-bros..mp3");
	...
	BattleShip* multiplayer = new BattleShip();
	...
	multiplayer->sprite = SheetSprite(spriteSheetTexture, 17, spriteCountX, spriteCountY, 3.0);
	...
	while (!done) {
		while (SDL_PollEvent(&event)) {
			if (event.type == SDL_QUIT || event.type == SDL_WINDOWEVENT_CLOSE) {
				...
				if (event.key.keysym.sym == SDLK_s){
					multiplayer->multiShootBullet();
					Mix_PlayMusic(enemyshootsound, 0);
				}
		...
		if (state == 0){
			...
			if (enemyLineOne.size() == 0 && enemyLineTwo.size() == 0 && enemyLineThree.size() == 0){
				...
				int enemyindex = 0;
				float enemysize = 1.0f;
				Vector3 enemySpeed = Vector3(2.0f, 0.0f, 0.0f);
				/**/
				// adding enemies to vectors
				for (float y = -1; y <= 3; y += 2){ // -1,3,2
					for (float i = -3; i <= 3; i += 2){
						if (y == -1){
							Vector3 enemyPosition = Vector3(float(i), float(y), 0.0f);
							Enemy* enemy = new Enemy(enemyPosition, enemySpeed);
				...
				OutputDebugString("state changed to 2 \n");
			}
		}
		else if (state == 2){
			// loop to add enemies
			...
			float elapsed = ticks - lastFrameTicks;
			lastFrameTicks = ticks;
			glClearColor(0.847, 0.749, 0.847, 1.0f); //0.4f, 0.2f, 0.4f, 1.0f
			glClear(GL_COLOR_BUFFER_BIT);
			program.setProjectionMatrix(projectionMatrix);
			program.setViewMatrix(viewMatrix);
			float fixedElapsed = elapsed;
			...
			for (int i = 0; i < enemyLineOne.size(); i++) {
				if (enemyLineOne[i] != nullptr){
					program.setModelMatrix(enemyMatrix);
					enemyMatrix.identity();
					enemyLineOne[i]->Update(fixedElapsed * 2, right, left, enemyLineOne, i);  // right, left
					if (enemyTime > 10){
						enemyLineOne[i]->position.y -= 1.0f;
						enemyLineOne[i]->speed.x += 2.0f;
					}
			...
			for (int i = 0; i < enemyLineTwo.size(); i++){
				program.setModelMatrix(enemyMatrix);
				enemyMatrix.identity();
				enemyLineTwo[i]->Update(fixedElapsed * 2, right, left, enemyLineTwo, i);  // right, left
				if (enemyTime > 10){
					enemyLineTwo[i]->position.y -= 1.0f;
					enemyLineTwo[i]->speed.x += 2.0f;
			...
			for (int i = 0; i < enemyLineThree.size(); i++){
				program.setModelMatrix(enemyMatrix);
				enemyMatrix.identity();
				enemyLineThree[i]->Update(fixedElapsed * 2, right, left, enemyLineThree, i);  // right, left
				if (enemyTime > 10){
					enemyLineThree[i]->position.y -= 1.0f;
					enemyLineThree[i]->speed.x += 2.0f;
					if (i == enemyLineThree.size() - 1){
			...
			if (enemyLineOne.size() == 0 && enemyLineTwo.size() == 0 && enemyLineThree.size() == 0){
				state = 3;
				OutputDebugString("state changed to 3 \n");
			...
			float elapsed = ticks - lastFrameTicks;
			lastFrameTicks = ticks;
			glClearColor(0.863f, 0.863f, 0.863f, 1.0f); //0.4f, 0.2f, 0.4f, 1.0f
			glClear(GL_COLOR_BUFFER_BIT);
			program.setProjectionMatrix(projectionMatrix);
			program.setViewMatrix(viewMatrix);
			float fixedElapsed = elapsed;
			...
			for (int i = 0; i < 1000; i++){
				...
				float vertices[] = { -0.125f, -0.125f, 0.125f, -0.125f, 0.125f, 0.125f, -0.125f, -0.125f, 0.125f, 0.125f, -0.125f, 0.125f };
				/**/
				if (bulletCollision(defender->bullets[i]->position.x, defender->bullets[i]->position.y, multiplayer->position.x, multiplayer->position.y)){
					//OutputDebugString("defender got hit \n");
					state = 4;
					break;
			...
			//mulitplayer
			program.setModelMatrix(multiplayermatrix);
			multiplayermatrix.identity();
			multiplayer->multiUpdate(fixedElapsed, right, left);
			multiplayermatrix.setPosition(multiplayer->position.x, multiplayer->position.y, multiplayer->position.z);
			multiplayer->sprite.Draw(&program);
			program.setModelMatrix(bulletMatrix);
			for (int i = 0; i < 1000; i++){
				bulletMatrix.identity();
				multiplayer->bullets[i]->multiUpdate(fixedElapsed);
				bulletMatrix.setPosition(multiplayer->bullets[i]->position.x, multiplayer->bullets[i]->position.y, multiplayer->bullets[i]->position.z);
				program.setModelMatrix(bulletMatrix);
				float vertices[] = { -0.125f, -0.125f, 0.125f, -0.125f, 0.125f, 0.125f, -0.125f, -0.125f, 0.125f, 0.125f, -0.125f, 0.125f };
				/**/
				if (bulletCollision(multiplayer->bullets[i]->position.x, multiplayer->bullets[i]->position.y, defender->position.x, defender->position.y)){
					//OutputDebugString("defender got hit \n");
					state = 5;
					break;

				}
				multiplayer->bullets[i]->sprite = SheetSprite(spriteSheetTexture, 25, spriteCountX, spriteCountY, 0.5);
				multiplayer->bullets[i]->sprite.Draw(&program);
			}
		}
		else if (state == 4){
